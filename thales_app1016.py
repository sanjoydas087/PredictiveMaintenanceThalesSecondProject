...
def load_data():
    ...
    try:
        # ── Read AE file from zip ─────────────────────────────
        # Compressed as ac.zip to keep GitHub file size small.
        # pd.read_csv() reads directly from zip — no extraction needed.
        import zipfile
        import io

        # Try ac.zip first, fall back to raw CSV if present
        if os.path.exists("ac.zip"):
            with zipfile.ZipFile("ac.zip", "r") as z:
                # Get the first CSV file inside the zip
                csv_name = [n for n in z.namelist() if n.endswith(".csv")][0]
                with z.open(csv_name) as f:
                    df_ae = pd.read_csv(io.BytesIO(f.read()))
        else:
    ...
    except (FileNotFoundError, IndexError, KeyError, Exception) as _ae_err:
        for col in ae_cols:
            df[col] = np.nan
