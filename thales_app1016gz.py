...
def load_data():
    ...
    # ── HELPER: read CSV from zip or directly ────────────────
    def read_csv_from_zip_or_file(zip_name, csv_fallback):
        """
        Try zip first (for Streamlit Cloud / GitHub deployment).
        Fall back to raw CSV for local development.
        zip_name    : e.g. "predictivemaintenancekpioutputcascade.zip"
        csv_fallback: e.g. "Predictive_Maintenance_KPI_Output_Cascade.csv"
        """
        if os.path.exists(zip_name):
            with zipfile.ZipFile(zip_name, "r") as z:
                # Get the first CSV inside the zip
                csv_names = [n for n in z.namelist() if n.endswith(".csv")]
                if not csv_names:
                    raise FileNotFoundError(f"No CSV found inside {zip_name}")
    ...
    # GitHub: predictivemaintenancekpioutputcascade.zip
    # Local : Predictive_Maintenance_KPI_Output_Cascade.csv
    ...
    # Sort by Machine_ID + Datetime BEFORE attaching AE columns
    # so both files have identical row order.
    ...
    # GitHub: ac.zip
    # Local : Autoencoder_Anomaly_Output.csv
    # Both files sorted identically (Machine_ID + Datetime)
    # so row 0 of AE = row 0 of cascade — guaranteed alignment.
    ae_cols = [
    ...
    try:
        ...
        # Attach only AE columns by row position
        ...
        # AE file not found or invalid — safe placeholders
        for col in ae_cols:
            df[col] = np.nan
...
with tab1:
    ...
    k3.markdown(kpi_card("High Risk Machines", high_risk_machines, "red",
                          f"{high_risk_pct}% of<br> fleet"), unsafe_allow_html=True)
...
with tab2:
    ...
    mk2.markdown(kpi_card("Anomaly Rate",
                           ...
                           "% readings flagged <br>anomalous"), unsafe_allow_html=True)

    mk3.markdown(kpi_card("Avg Lead Time", f"{m_avg_lead} hrs", "blue",
                           "Hours before <br>predicted failure"), unsafe_allow_html=True)

    mk4.markdown(kpi_card("Cascade Failure Index", m_avg_cascade, "amber",
                           "Risk of failure<br> propagation"), unsafe_allow_html=True)
