...
def load_data():
    ...
    # ── STEP 1: Load the main Cascade KPI file ───────────────
    # This is always the source of truth for base data.
    # It has exactly 100,000 rows with correct date range.
    df = pd.read_csv("Predictive_Maintenance_KPI_Output_Cascade.csv")

    # ── STEP 2: Parse and sort by Datetime immediately ───────
    # Sort BEFORE attaching AE columns so both files
    ...
    # ── STEP 5: Attach AE columns by row position ────────────
    # AE file was generated from the SAME base data sorted by
    # Machine_ID + Datetime. We sort both files identically
    # so row 0 of AE = row 0 of cascade guaranteed.
    ae_cols = [
    ...
    try:
        ...
        # The large AE output is stored as ac.zip instead of a raw CSV.
        # Read the CSV directly from the ZIP without extracting it to disk.
        zip_path = "ac.zip"
        csv_name = "Autoencoder_Anomaly_Output.csv"
        ...
        with zipfile.ZipFile(zip_path, "r") as z:
            ...
            # First look for the expected exact filename.
            matching = [
                m for m in members
                if os.path.basename(m) == csv_name
            ]

            if not matching:
                # Fallback: find any CSV whose name contains "Autoencoder".
                matching = [
                    m for m in members
                    if m.lower().endswith(".csv")
                    and "autoencoder" in os.path.basename(m).lower()
                ]

            if not matching:
                raise FileNotFoundError(
                    f"{csv_name} was not found inside {zip_path}"
                )

            with z.open(matching[0]) as ae_file:
                df_ae = pd.read_csv(ae_file)

        df_ae["Datetime"] = pd.to_datetime(
            df_ae["Date"].astype(str) + " " + df_ae["Timestamp"].astype(str),
            dayfirst=True, errors="coerce"
        )

        # Sort AE file SAME way as main file
        df_ae = df_ae.sort_values(
            ["Machine_ID", "Datetime"]
        ).reset_index(drop=True)

        # Attach only AE columns — use first n rows
        ...
        df["AE_Model_Loaded"] = len(ae_cols_present) > 0
    ...
    # ── STEP 6: Final sort by Datetime for time-series charts ─
...
with st.sidebar:
    ...
    if not AE_AVAILABLE:
        st.markdown(
            ...
            f'Upload ac.zip beside thales_app.py<br>'
            f'and keep Autoencoder_Anomaly_Output.csv inside it</div>',
            unsafe_allow_html=True
        )
...
# ── PRE-COMPUTE ALL FINDING VALUES ───────────────────────────

# Tab 1
kf_risk_counts   = dashboard_df["Predictive_Maintenance_Risk"].value_counts()
...
kf_hr_pct        = round(kf_hr_count / len(dashboard_df) * 100, 1)
kf_mr_pct        = round(kf_mr_count / len(dashboard_df) * 100, 1)
...
kf_op_risk       = dashboard_df.groupby("Operation_Mode")["Anomaly_Score"].mean().sort_values(ascending=True)
kf_riskiest_mode = kf_op_risk.index[0]
kf_riskiest_score= round(kf_op_risk.iloc[0], 4)

kf_imm_action    = (dashboard_df["Maintenance_Action"] == "Immediate Maintenance Required").sum()

# Tab 2
kf_m_anom_rate   = round((machine_data["Anomaly_Label"] == -1).mean() * 100, 2)
kf_m_avg_score   = round(machine_data["Anomaly_Score"].mean(), 4)
kf_m_lead_time   = round(machine_data["Early_Warning_Lead_Time_Hours"].mean(), 1)
kf_m_anom_count  = int((machine_data["Anomaly_Label"] == -1).sum())
kf_corr_cols     = ["Temperature_C", "Vibration_Hz", "Power_Consumption_kW", "Error_Rate_%", "Anomaly_Score"]
kf_corr          = machine_data[kf_corr_cols].corr()
...
kf_m_sev         = machine_data["Anomaly_Severity"].value_counts()
kf_m_top_sev     = kf_m_sev.idxmax()
kf_m_top_sev_pct = round(kf_m_sev.max() / kf_m_sev.sum() * 100, 1)

# Tab 3
kf_casc          = dashboard_df["Cascade_Risk_Level"].value_counts()
kf_hi_casc       = kf_casc.get("High Cascade Risk", 0)
kf_hi_casc_pct   = round(kf_hi_casc / len(dashboard_df) * 100, 1)
kf_casc_top      = dashboard_df.groupby("Machine_ID")["Cascading_Failure_Index"].mean().sort_values(ascending=False)
kf_top_casc_m    = f"M-{kf_casc_top.index[0]}"
kf_top_casc_v    = round(kf_casc_top.iloc[0], 2)

# Tab 4
kf_fleet_lt_mean  = round(dashboard_df["Early_Warning_Lead_Time_Hours"].mean(), 1)
kf_fleet_lt_min   = round(dashboard_df["Early_Warning_Lead_Time_Hours"].min(), 1)
kf_critical_lt    = int((dashboard_df["Early_Warning_Lead_Time_Hours"] <= 25).sum())
kf_escalating_flag= int((machine_data["Risk_Trend_Status"] == "Escalating").sum())
kf_improving_flag = int((machine_data["Risk_Trend_Status"] == "Improving").sum())
kf_maint_data_kf  = machine_data[machine_data["Operation_Mode"] == "Maintenance"]
kf_active_data_kf = machine_data[machine_data["Operation_Mode"] != "Maintenance"]
...
with tab3:
    ...
    cfp1.markdown(kpi_card(
        ...
        f"{active_count:,}",
        "red",
        f"{active_pct}% — All 3 conditions met. Immediate action."
    ), unsafe_allow_html=True)
    ...
    cfp3.markdown(kpi_card(
        ...
        f"{no_pat_count:,}",
        "green",
        "Normal behaviour. Continue monitoring."
    ), unsafe_allow_html=True)
...
with tab5:
    ...
    if not AE_AVAILABLE:
        ...
        with c_bp:
            fig, ax = styled_fig(figsize=(7, 4.2))
            bp = ax.boxplot(
                ae_by_risk, labels=risk_order, patch_artist=True,
                widths=0.45, medianprops=dict(color=TEXT_PRI, linewidth=2)
            )
...
with col_info:
    ...
    <div style="font-size:0.80rem;color:{TEXT_SEC};line-height:1.8;">
      ...
      <b style="color:{TEXT_PRI};">AE Deployment:</b>
      Autoencoder output loaded directly from <b>ac.zip</b><br>
